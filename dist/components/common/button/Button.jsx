...
const Button = forwardRef(function Button({ variant = 'primary', size = 'md', isLoading = false, leftIcon, rightIcon, children, className = '', ...rest }, ref) {
    ...
    return (<motion.button ref={ref} className={`${classNames.trim()}focusable`} disabled={isLoading || rest.disabled} whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.02 }} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 200, damping: 18 }} {...rest}>
