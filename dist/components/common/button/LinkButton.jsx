...
const LinkButton = forwardRef(function LinkButton({ href, variant = 'primary', size = 'md', isLoading = false, leftIcon, rightIcon, children, className = '', prefetch = true, 'aria-label': ariaLabel, ...rest }, ref) {
    const classNames = [
        ...
        .filter(Boolean)
        .join(' ');
    return (<NextLink href={href} prefetch={prefetch} className={`${classNames} focusable`} aria-label={ariaLabel} aria-disabled={isLoading} tabIndex={isLoading ? -1 : 0} ref={ref} {...rest}>
