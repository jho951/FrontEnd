...
function Header({ adOffset }) {
    ...
    return (<>
      ...
      <motion.header className={`${styles.header} ${menuOpen && styles.open}`} initial={false} animate={{ opacity: isScrolling ? 0.15 : 1 }} transition={{ opacity: { duration: isScrolling ? 0.4 : 0.8, ease: 'easeInOut' } }} style={{
        ...
        <section className={styles.topRow}>
          ...
          <div className={styles.btnContainer}>
            <HamburgerButton isOpen={menuOpen} size={24} onClick={() => setMenuOpen(v => !v)} tabIndex={0}/>
          </div>
        </section>
